...
(function (window, undefined) {
  ...
  var Hustle = function (qoptions) {
    ...
    var maintenance_delay = qoptions.maintenance_delay
      ? qoptions.maintenance_delay
      : 1000;
    ...
    var check_db = function () {
      if (!db)
        throw new HustleDBClosed(
          'Closed database being operated on. Did you call Hustle.open()?'
        );
      return true;
    };
    ...
    /**
     * clear jobs from all tubes
     */
    var clearHustle = function (tube) {
      var tr = db.transaction([tube], 'readwrite');
      var store = tr.objectStore(tube);
      store.clear();
    };

    var generateDeferredPromise = function () {
      let resolve;
      let reject;
    ...
    /**
     * insert data row in the respective tube
     */
    var insertDataInHustle = function (data, tube) {
    ...
    var dbInfo = function () {
      var stores = [];
      var storeNames = Object.values(db.objectStoreNames);
    ...
    /**
     * return all tubes present in HustleDB
     */
    ...
    /**
     * return data from each tube
     */
    var backupAll = async function (storeName) {
      var req,
        results = [];
      var d = generateDeferredPromise();
    ...
    var move_item = function (id, from, to, options) {
      ...
      req.onsuccess = function (e) {
        ...
        if (!item) {
          if (options.error)
            options.error(new HustleNotFound('item ' + id + " wasn't found"));
          return;
        }
    ...
    var peek = function (id, options) {
      ...
      var tables = [tbl.reserved, tbl.delayed, tbl.buried].concat(
        qoptions.tubes
      );
      ...
      trx.oncomplete = function (e) {
        if (!item && options.not_found_error) {
          if (options.error)
            options.error(new HustleNotFound('item ' + id + ' not found'));
          return;
        }
      ...
      tables.forEach(function (table) {
        ...
        req.onsuccess = function (e) {
          ...
          if (table == tbl.reserved) {
            item.state = 'reserved';
            if (item.ttr > 0) {
              item.time_left = Math.round(
                (item.expire - new Date().getTime()) / 1000
              );
            }
    ...
    var put = function (data, options) {
      ...
      if (qoptions.tubes.indexOf(tube) < 0)
        throw new HustleBadTube('tube ' + tube + " doesn't exist");
      ...
      var createItem = function () {
        ...
        new_id({
          ...
          error: function (e) {
            if (options.error)
              options.error(new HustleBadID('error generating id'));
          },
        });
      };
      ...
      peek(id, {
        ...
        success: function (item) {
          if (item.state != 'reserved') {
            if (options.error)
              options.error(
                new HustleNotFound('item ' + id + " isn't reserved")
              );
            return;
          }
    ...
    var kick_job = function (id, options) {
      ...
      peek(id, {
        ...
        success: function (item) {
          if (item.state != 'buried') {
            if (options.error)
              options.error(new HustleNotFound('item ' + id + " isn't buried"));
            return;
          }
    ...
    var rescue_reserved_items = function (options) {
      ...
      var DEFAULT_RESCUE_LIMIT = 5,
        DEFAULT_RESCUE_TIME_LIMIT_IN_SECONDS = 60;
      var maxRescueLimit = options.maxRescueLimit || DEFAULT_RESCUE_LIMIT;
      var rescueTimeLimitInSeconds =
        options.rescueTimeLimitInSeconds ||
        DEFAULT_RESCUE_TIME_LIMIT_IN_SECONDS;
      ...
      var move_items_in_reserved = function () {
        var move_item_to_tube = function (item) {
          ...
          var should_increase_abandon_count = function (item) {
            ...
            var timeInSecondsAfterLastAbandoned =
              (new Date().getTime() - item.lastAbandonedTime) / 1000;
        ...
        items_in_reserved.forEach(function (item) {
          var shouldRescue =
            !item.abandon_count || maxRescueLimit > item.abandon_count;
          if (shouldRescue) {
    ...
    this.open = open;
    this.clearHustle = clearHustle;
    this.insertDataInHustle = insertDataInHustle;
    ...
    this.promisify = function () {
      ...
      this.Queue.cleanup_abandoned_items = do_promisify(
        this.Queue.cleanup_abandoned_items,
        0
      );
      this.Queue.rescue_reserved_items = do_promisify(
        this.Queue.rescue_reserved_items,
        0
      );
      return this;
    }.bind(this);
